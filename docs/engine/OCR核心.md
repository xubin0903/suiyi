...
- **运行参数（#74，`OcrRuntimeOptions`，默认 `DEFAULT_RUNTIME`）**：RapidOCR 没有「只缩小检测图」的参数（`Global.max_side_len` 会连识别裁切一起缩小），
  ...
  3. （在第 2 步裁切之前）同一行上相接或重叠的横排框（左右间距 ≤ 0.25 × 行高、上下重叠 ≥ 0.7 × 行高）先合成一个再识别（#75，`merge_touching_boxes`）：关闭膨胀后检测偶尔把 `return ""` 切成重叠的两个框，引号单独识别成 `" I`。有明显空隙的碎片（菜单项之间）不合并，交给分段合成一行。
  4. 返回给分段的行框沿短边每侧收回 `box_shrink × (1/scale − 1)` 像素（缩放补偿：DB 外扩在检测图上是常数像素，换回原图被放大，会吃掉段间距）。不缩放时为 0。
...
按从上到下处理每一行，在已有段落里找「上一行」与它横向有重叠、行距在 `[-0.5, 0.9] × 字号` 内、最近的一个。找到后再按顺序检查（阈值都在 `LayoutOptions`，括号里是字段名与默认值；长度以字号为单位，横排字号 = 行框高）：

| 顺序 | 条件 | 不满足时 |
| --- | --- | --- |
| 1 | 字号比 ≤ 1.35（`height_ratio`）。两种豁免：新行只有一个字（单字框尺寸随字形变化，#74）；**段尾短行**（#75，见下） | 标题与正文，分段 |
| 2 | 行首比上一行多缩进 ≤ 1 × 字号（`indent`） | 首行缩进，新段 |
| 3 | 行首与上一行对齐（容差 0.8 × 字号，`align`）；段落只有一行且新行更靠左时例外（上一行是缩进的首行） | 分段 |
| 4 | 上一行以**续接标点**结尾 → 直接并入（跳过 5、6） | — |
| 5 | **短行分段**（#75，只对横排）：相邻两行的长度都 ≤ 12 × 各自字号（`short_row`） | 两个独立的 UI 条目，分段 |
| 6 | 上一行行尾不比段落右缘短 2 × 字号以上（`short_line`）；下一行以小写拉丁字母开头时放宽到 4 × 字号（#75：西文换行时两行差一个词很常见） | 上一行是段尾（或短标题），分段 |

**续接标点**（`CONTINUATION_END`）：`，、,；：（(《「『—-`。行尾是这些字符说明句子没写完（「目标语言：简体中文，」+「检测到中文时改译英文」）。
不含半角 `:` `;`：代码行常以它们结尾，但每行各自独立。

**短行分段**（#75）：UI 截图里菜单项、侧栏、设置项、列表项的行距和正文一样，行距阈值分不开（#54：line_gap 0.9 → 0.6 时 UI 误合并只从 13 降到 12）。
它们的共同点是**相邻两行都很短**；正文段落里除了最后一行，其余行都接近栏宽。所以相邻两行都 ≤ 12 × 字号、且上一行不以续接标点结尾时分段。
12 × 字号约等于 12 个汉字或 20–25 个西文字符。`short_row` 在 dev 集上 10–14 的结果相同（误拆分都是 1）；16 起误拆分开始增加，20 时正文窄栏被大量拆开。取 12，离两边都有余量。
这条规则会把**窄栏里的正文**（每行 ≤ 12 个字）拆开，也分不开长度超过 12 × 字号的 UI 行（比如整行菜单栏和状态栏）。竖排不启用。

**段尾短行**（#75）：缩小检测图后，只有几个字的段尾行（「对。」）行框常比上一行高 1.5 倍左右，会被字号比当成标题拆出去（#74 遗留的两处误拆分）。
满足以下全部条件时，不做字号比检查：
- 本段已有 ≥ 2 行；
- 新行比上一行短，长度 ≤ `short_row` × 字号；
- 字号比 ≤ 1.7（`tail_ratio`）；
- 与上一行的间距不大于本段已有行距 + 0.1 × 字号。标题前通常有段间距，这一条把标题排除在外。
